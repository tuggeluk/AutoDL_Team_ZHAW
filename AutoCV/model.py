...
class Model(object):
  ...
  def __init__(self, metadata):
    ...
    self.temporal_kernel = 7
    self.number_of_frames = 8
    self.stride = 1
    self.variation_selection = True
    self.testing = False
  ...
  def preprocess_tensor_4d(self, tensor):
    ...
    if self.input_shape[0]==-1 or self.input_shape[1]==-1 or self.input_shape[0] > 90 or self.input_shape[1] > 90:
    #if True:
        input_shape = self.default_image_size
        self.input_shape = self.default_image_size
    if self.random_crop and not self.testing:
       input_shape = [150,150]
    tensor_shape = tensor.shape#  get_shape().as_list()
    pad_frames = self.stride * frames + 1

    if True:#not self.variation_selection or (self.variation_selection and not self.testing):
      maxval = tf.math.maximum(tf.shape(tensor)[0] - self.stride * frames + 1, 1)
      begin_index = tf.cond(tf.shape(tensor)[0] - self.stride * frames + 1 < 1, lambda: 0, lambda: tf.random.uniform([1], minval=0, maxval=maxval, dtype=tf.int32)[0])
    else:
    # find begin_index
      #step_size = tf.cast(tf.math.maximum(tf.cast(1.0, tf.float64),tf.shape(tensor)[0]/20),dtype=tf.int64)
      #tensor_slice = tf.reshape(tensor[0:tf.shape(tensor)[0]:step_size, :, :, :], [-1, self.input_shape[0], self.input_shape[1], tensor.get_shape().as_list()[-1]])
      variation = tf.image.total_variation(tensor)
      begin_index = tf.math.maximum(tf.cast(0, tf.int64), tf.math.minimum(tf.math.argmax(variation), tf.cast(tf.shape(tensor)[0] - pad_frames, tf.int64)))
      begin_index = tf.cast(begin_index,dtype=tf.int32)
    end_index = begin_index + self.stride * frames

    tensor = tf.cond(tf.logical_not(tf.logical_and(tf.is_nan(tf.cast(tf.shape(tensor)[0], tf.float32)), tf.cast(tf.shape(tensor)[0], tf.float32) < pad_frames)), lambda: tf.tile(tensor, [tf.cast(tf.math.floor(pad_frames / tf.cast(tf.shape(tensor)[0], tf.float32)) + 1, tf.int32), 1, 1, 1]), lambda: tensor)
    tensor = tf.cond(tf.logical_not(tf.logical_and(tf.is_nan(tf.cast(tf.shape(tensor)[0], tf.float32)), tf.cast(tf.shape(tensor)[0], tf.float32) > pad_frames)), lambda: tensor[begin_index:end_index:self.stride, :, :, :], lambda: tensor)
  ...
  def model(self, features):
    ...
    if self.metadata.get_tensor_shape()[0] != -1:
        ...
        if self.metadata.get_tensor_shape()[-1] == 1:
        ...
        with tf.variable_scope("FullyConnected"):
          ...
          #hidden_layer = tf.nn.relu(tf.subtract(hidden_layer, tf.expand_dims(tf.contrib.distributions.percentile(hidden_layer, 0.1, -1), -1)))
        ...
        '''
        with tf.variable_scope("FullyConnected"):
          #hidden_layer = tf.contrib.layers.conv2d(inputs=endpoints['feature_maps'], num_outputs=1280, kernel_size=1, stride=1, activation_fn=None)
          ...
          temporal_kernel_size = [self.temporal_kernel, 3, 3]
          temporal_stride =  [1, 1, 1]
          temporal_conv = tf.keras.layers.Conv3D(filters=8, kernel_size=temporal_kernel_size, strides=temporal_stride, padding="same", use_bias=True)
          hidden_layer = temporal_conv(hidden_layer)
          hidden_layer = tf.reduce_mean(hidden_layer, axis=[2, 3])
          ...
          hidden_layer = tf.layers.dense(inputs=hidden_layer, units=256, activation=tf.nn.relu)
          #hidden_layer = tf.layers.dropout(inputs=hidden_layer, rate=0.0, training=self.is_training)
          logits = tf.layers.dense(inputs=hidden_layer, units=self.output_dim)
  ...
  def TrainOp(self, loss, scope=None, learning_rate=1e-2, momentum=0.9, optimizer=tf.train.MomentumOptimizer):
    ...
    with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
        train_op = optimizer.minimize(loss=loss, var_list=tf.global_variables())
    return train_op
  ...
  def train(self, dataset, remaining_time_budget=None):
    ...
    if self.train_iterator_handle is None:
      ...
      if True:#self.metadata.get_tensor_shape()[1] == -1:
        ...
        try:
          for i in range(2*self.output_dim):
            # load full dataset
        ...
        A = np.mean(np.sum(np.diff(examples_labels, axis=0)!=0, 0))
        print("Measured statistics for radnom shuffle: {:.4f}, output dimensions :{:.4f}".format(A, self.output_dim))
        if A < 0.3:
          self.shuffle = True
        ...
        self.mean = np.average(shapes, 0)
        self.mean_sizes = self.mean[1:3]
        self.mean_sequence = self.mean[0]
        shape = self.mean_sizes
        self.mean_sizes = self.mean_sizes.astype(np.int)
        self.mean_sequence = self.mean_sequence.astype(np.int)
        if self.mean_sequence > 1.0 and self.mean_sequence <= 6*self.number_of_frames:
          self.number_of_frames = 4
          self.stride = 1
          self.temporal_kernel = 3
        elif self.mean_sequence > 6*self.number_of_frames:
          #self.number_of_frames = 4
          self.stride = 4
          #self.temporal_kernel = 3
      else:
      ...
      latest_checkpoint = tf.train.latest_checkpoint( _HERE()+"/pretrained_models")
      ...
      self.trainOp0 = self.TrainOp(self.loss, scope="FullyConnected", learning_rate=self.learning_rate)
    ...
    sample_count = 0
    if self.FirstIteration:
      self.training_steps = int(np.round(np.min([1e1*256/self.batch_size+1, self.metadata.size()/self.batch_size+1])/2))
    else:
      self.training_steps *= 2.5
      self.training_steps = min([self.training_steps, 200])
    start_training = time.time()
    while sample_count < self.training_steps: # when do we return? TODO come up with smart heuristic
      try:
        if self.FirstIteration:
          if self.metadata.get_tensor_shape()[0] != -1:
            for d in range(int(min([30, self.metadata.size()/self.batch_size+1]))):
            ...
            for d in range(int(min([10, self.metadata.size()/self.batch_size+1]))):
              _loss, _labels, _predictions, _logits, _ = self.tf_session.run([self.loss, self.labels, self.predictions, self.logits, self.trainOp0], feed_dict={self.iterator_feed_handle: self.train_iterator_handle, self.is_training: True})
              sample_count += 1
            for d in range(int(min([30, self.metadata.size()/self.batch_size+1]))):
              _loss, _labels, _predictions, _logits, _ = self.tf_session.run([self.loss, self.labels, self.predictions, self.logits, self.trainOp0], feed_dict={self.iterator_feed_handle: self.train_iterator_handle, self.is_training: True})
  ...
  def test(self, dataset, remaining_time_budget=None):
    ...
    if self.test_iterator_handle is None:
      ...
      dataset = dataset.batch(int(4*self.batch_size)).prefetch(1)
