...
class ResNet18(models.ResNet):
    ...
    def __init__(self, in_channels, num_classes=10, **kwargs):
        ...
        self.conv3d = torch.nn.Conv3d(in_channels=self.last_channels, out_channels=self.last_channels, kernel_size=[7,2,2], stride=[1,1,1],padding_mode="same", bias=True)
        ...
        self.conv1d = torch.nn.Sequential(
            ...
            torch.nn.Conv1d(self.last_channels, self.last_channels//2,
                            kernel_size=5, stride=1, padding=2, bias=False),
            torch.nn.BatchNorm1d(self.last_channels//2),
            torch.nn.ReLU(inplace=True),
        ...
        self.pool3d = torch.nn.AdaptiveAvgPool3d((1, 1, 1))
        # self.pool = torch.nn.AdaptiveMaxPool2d((1, 1))
        self.fc1 = torch.nn.Linear(self.last_channels, self.last_channels, bias=True)
    ...
    def forward_origin(self, x):
        ...
        if self.is_video():
            ...
            # x = x.view(x.size(0), 1, x.size(1), x.size(2),x.size(2))
